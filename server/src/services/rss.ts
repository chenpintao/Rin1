...
function getConfiguredFrontendUrl(env: Env): string | null {
...
async function handleFeed(c: AppContext, fileName: string) {
    ...
    const configuredFrontendUrl = getConfiguredFrontendUrl(env);
    const canUsePersistentCache = Boolean(configuredFrontendUrl);

    // A storage cache has no host dimension. Without a configured canonical URL,
    // generate from the current request origin instead of serving another host's feed.
    ...
    try {
        ...
        const frontendUrl = configuredFrontendUrl || new URL(c.req.url).origin;
...
export async function rssCrontab(env: Env, db: DB) {
    const frontendUrl = getConfiguredFrontendUrl(env);
    if (!frontendUrl) {
        console.log('[RSS] Skipping pre-generation because FRONTEND_URL is not configured');
        return;
    }

    // Generate feed
    // For cron jobs, we don't have a request context, so fall back to the configured
    // frontend URL. The RSS feeds generated by cron jobs will be stored in S3 and served from there
